...
class PahoMQTT(MQTTBroker):
    ...
    def _on_connected(self, client, userdata, flags, rc) -> None:
        ...
        logging.debug("Connected result={0}, flags={2}".format(rc, flags))
        return
    ...
    def connect(self) -> None:
        result = self._client.connect(self._endpoint, self._port, keepalive=120)
        if result != 0:
    ...
    def publish(self, topic: str, payload: typing.Dict[str, any]) -> None:
        ...
        result = self._client.publish(topic=topic, payload=json.dumps(payload), qos=1)
        try:
...
class IotCoreMQTT(MQTTBroker):
    ...
    def connect(self) -> None:
        ...
        try:
            ...
            logging.error("Connection to AWS IoT Core failed for unknown reason")
            raise err
